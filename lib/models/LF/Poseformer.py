...
class PoseTransformerV2(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2,
        ...
        self.num_frame_kept = 3
        self.num_coeff_kept = 3
    ...
    def Spatial_forward_features(self, x):
        ...
        x = self.Joint_embedding(x[:, index].view(b*num_frame_kept, p, -1))             # [B*3, J, dim]
        ...
        x = rearrange(x, '(b f) p c -> b f (p c)', f=num_frame_kept)                    # [B, t, J*dim]
        return x
    ...
    def forward_features(self, x, Spatial_feature):
        ...
        x = dct.dct(x.permute(0, 2, 3, 1))[:, :, :, :num_coeff_kept]        # [B, T, J, 2] -> [B, J, 2, T]
        x = x.permute(0, 3, 1, 2).contiguous().view(b, num_coeff_kept, -1)  # [B, t, J*2]
        x = self.Freq_embedding(x)                                          # [B, t, dim]
        
        Spatial_feature += self.Temporal_pos_embed                          # [B, 3, dim]
        x += self.Temporal_pos_embed_
        x = torch.cat((x, Spatial_feature), dim=1)                          # [B, t+3, dim]
    ...
    def forward(self, x):
        ...
        x = self.forward_features(x_, Spatial_feature)  # [B, t+3, dim]
        
        x = torch.cat((self.weighted_mean(x[:, :self.num_coeff_kept]), self.weighted_mean_(x[:, self.num_coeff_kept:])), dim=-1) # [B, 1, 2dim]
        # [B, t, dim] -> [B, 1, dim] , [B, 3, dim] -> [B, 1, dim]

        x = self.head(x).view(b, 1, p, -1)  
        return x

if __name__ == "__main__" :
    x = torch.rand((1, 16, 17, 2))
