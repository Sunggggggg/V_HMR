...
class FreqTempEncoder(nn.Module) :
    def __init__(self, num_joints, embed_dim, depth, num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
        ...
        self.weighted_mean_ = torch.nn.Conv1d(in_channels=num_coeff_keep, out_channels=1, kernel_size=1)
    ...
    def forward(self, full_2d_joint, short_2d_joint, num_imgs=3):
        ...
        x = torch.cat((self.weighted_mean(joint_feat), self.weighted_mean_(freq_feat)), dim=-1) # [B, 1, 2dim]

        x = self.head(x).view(B, 1, J, -1) 
        return x